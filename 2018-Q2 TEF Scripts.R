...
##########################################
con <- dbConnect(odbc::odbc(),.connection_string = 'driver={SQL Server};server=blndb11;database=DE_PreBM_Voice_02_backuped;trusted_connection=true')
...
kpiReport <-    dbGetQuery(con,paste("SELECT  Rnk
                                     ...
                                     FROM NEW_VDF_KPI_REPORT_OPERATOR_3"))
...
write.csv(kpiReport, file = "TEF-Q2-FinalTable.csv")
# Disconnect Database
##########################################

...
###################################################################################################################################################
overallRnkPlot <- function(fn,rnk,level,operator)
{
  if(nrow(rnk) < 5){
    horRs <- nrow(rnk)*1000
  } else{
  ...
  file <- png(file=fn,width=400,height=horRs,res=72)
  par(mfrow = c(6,1))
  
  names <- paste(rnk$G_Level_1,"\n",rnk$G_Level_2)
  
  plotBarCum1( inputTab    = t(cbind( rnk$ranVoice, rnk$ranData )), 
               inputHead   = paste("Total Ranking", operator), 
               inputNam    = names, 
               inputColo   = c( rgb(148, 171,  33, maxColorValue=255),
                                rgb( 78, 166, 220, maxColorValue=255) ), 
               inputKP     = c("Voice", "Data"), 
               legFlag     = "Yes", 
               inputLabP   = t(cbind( rnk$ranData + rnk$ranVoice ) ),
               inputLab    = paste( round( (rnk$ranData + rnk$ranVoice ), digits = 2 ),
                                    "of",
               ...
               mi          = max(rnk$ranData + rnk$ranVoice) )
  
  plotBarCum1( inputTab    = t(cbind( rnk$lostVoice, rnk$lostData )), 
               inputHead   = paste("Points Lost", operator), 
               inputNam    = names, 
               inputColo   = c( rgb(148, 171,  33, maxColorValue=255),
                                rgb( 78, 166, 220, maxColorValue=255) ), 
               inputKP     = c("Voice", "Data"), 
               legFlag     = "Yes", 
               inputLabP   = t(cbind( rnk$lostData + rnk$lostVoice ) ),
               inputLab    = paste( round( (rnk$lostData + rnk$lostVoice ), digits = 2 ),
                                    "of",
                                    (rnk$maxData + rnk$maxVoice),
                                    "\n",
               ...
               mi          = max(rnk$lostData + rnk$lostVoice) )
  dev.off()
}
...
overallRnkPlot("graph1.png",rnk1,1,"Telekom")
